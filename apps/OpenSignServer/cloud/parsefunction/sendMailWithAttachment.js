...
async function sendMailProvider(params) {
  ...
  try {
    ...
    const mailsenderFallback = mailgunApiKey ? process.env.MAILGUN_SENDER : process.env.RESEND_SENDER;
    if (params.url) {
      ...
      try {
        ...
        if (ress) {
          ...
          const mailsender = smtpenable ? process.env.SMTP_USER_EMAIL : mailsenderFallback;
          ...
          if (transporterSMTP) {
            ...
            const resendAttachments = attachment.map(a => ({
              filename: a.filename,
              content: (a.content || a.data).toString('base64'),
            }));
            const resendRes = await fetch('https://api.resend.com/emails', {
              ...
              body: JSON.stringify({
                ...
                cc: messageParams.cc ? [messageParams.cc] : undefined,
